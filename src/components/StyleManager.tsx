...
import { checkFontAvailability, type FontAvailability } from '@/lib/fonts'
  ...
  return (
    <div>
      <select value={value} onChange={e => onChange(e.target.value)}
        className={`w-full border rounded px-2 py-1 text-sm ${currentMissing ? 'border-orange-300 bg-orange-50' : ''}`}>
        {fonts.map(f => (
      ...
      {currentMissing && (
        <p className="text-[10px] text-orange-500 mt-0.5">이 시스템에 설치되지 않은 폰트입니다</p>
      )}
    </div>
  )
}
